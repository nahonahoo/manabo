...
// 名古屋駅まわりの密集駅だけ視認性のために少しずらしてある（まなぼ側と同じ座標）
const STATION_POS = {
...
// コース内容・値段はまなぼ側と完全に同じ（実在の路線・車両をそのままモデルにしているため）
const TRAIN_COURSES = [
...
// ── 車両メダル用データ（実在の名前・実在に近い配色でデザイン。まなぼ側と同じ） ──
const VEHICLE_TYPES = {
...
function renderRouteMap() {
  ...
  // 実寸を大きめに描画して、指でスクロール・ピンチしながら見られるようにする（viewBoxは900x700のまま）
...
function renderCourseShop() {
  ...
  return TRAIN_COURSES.map(course => {
    ...
    return `<div style="background:#fdf8f0;border:1px solid #e0d5c0;border-radius:10px;padding:10px 12px;display:flex;flex-direction:column;gap:6px">
      ...
      <div style="font-size:.68rem;color:#c06010">🚃 ${esc(vehicleNames)}${newLines.length > 0 ? ` ・ 🎖ろせんメダル${newLines.length}こ` : ''}${newVehicles.length > 0 ? ` ・ 🎖のりものメダル${newVehicles.length}こ` : ''}</div>
...
function playTrainAnimation(course) {
  return new Promise(resolve => {
    ...
    const vehicleNames = course.vehicles && course.vehicles.length ? course.vehicles.map(id => VEHICLE_TYPES[id].name).join('・') : 'でんしゃ';
...
async function finishTrainCourse(course) {
  ...
  if (gotCount > 0) {
    showToast(`✨「${course.name}」でたびにでて、きねんメダルを${gotCount}こゲットしたよ！`);
    const names = [...newMedals.map(l => l.name), ...newVehicles.map(id => VEHICLE_TYPES[id].name)];
    typeText(`わあ！${names.join('・')}にのったよ！`);
  } else {
